...
/*	OLD IMPLEMENTATION
	...
	public void update(final double simTimeInSec) {
		...
		if (c.getElements().size() > 0) {
			// convert the infection rate into a time step unit
			double timeStepLength = simTimeInSec - this.lastSimTimeInSec;
			this.lastSimTimeInSec = simTimeInSec;
			double infectionRatePerTimeStep = attributesSIRG.getInfectionRate() * timeStepLength;
			...
			for(Pedestrian p : c.getElements()) {
				...
				for (Pedestrian p_neighbor : p_neighbors) {
					if (p == p_neighbor || getGroup(p_neighbor).getID() != SIRType.ID_INFECTED.ordinal())
						continue;
					if (this.random.nextDouble() < infectionRatePerTimeStep) {
						SIRGroup g = getGroup(p);
						if (g.getID() == SIRType.ID_SUSCEPTIBLE.ordinal()) {
							elementRemoved(p);
							assignToGroup(p, SIRType.ID_INFECTED.ordinal());
